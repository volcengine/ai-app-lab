...
import {
  type SSEError,
  sendMessageAndUpdateState,
} from './sendMessageAndUpdateState';
...
export const ChatWindowV2 = ({
  ...
  const addMessage = useCallback(
    (role: 'user' | 'bot', options: MessageOptions): void => {
  ...
  const insertBotEmptyMessage = (character?: {
    name?: string;
    avatar?: string;
  }) => {
    // 新增 BotMessage
    ...
    addMessage('bot', {
      versions: [
        [genSpecificMessage.loading(), genSpecificMessage.slot(newMessageId)],
      ],
      character,
    });
  };
  ...
  const updateAssistantResponse: (
    ...
    (
      ...
      ({
        ...
        const lastBotMessage = messages[lastBotMessageIndex] as
          | BotMessage
          | undefined;
        ...
        const targetMsgVersions = [
          ...lastBotMessage.versions[lastBotMessage.currentVersion],
        ];

        const targetMsgIdx = targetMsgVersions.findIndex(
        ...
        const isHasContent =
          !showSearching &&
          Boolean(
            content.startsWith(searchingText) ? content.slice(4) : content,
          );
        const updatedLoadingMsg = updateLoadingMessage(
          loadingMsg,
          showSearching,
          isHasContent,
        );
        ...
        const isUseSearch =
          ...
          updatedLoadingMsg.content === searchingText;
        ...
        // 拼装versions
        const updatedVersions = updateVersionsWithExtrasAndMessages(
          updatedLoadingMsg,
          updatedTargetMsg,
        );

        // 最终更新的数据
        const newLastAssistantMessage = updateBotMessageVersions(
          lastBotMessage,
          updatedVersions,
          finish,
          extra,
        ...
        setMessages(prevMessages =>
          prevMessages.map((message, index) =>
            index === lastBotMessageIndex ? newLastAssistantMessage : message,
          ),
        );
        ...
        if (finish) {
          ...
          const msgs = messages.map((msg, index) =>
            index === lastBotMessageIndex ? newLastAssistantMessage : msg,
          );
  ...
  const startReply = async ({
    target,
    consumeRate,
  }: { target?: string; consumeRate?: boolean }) => {
    ...
    const body = {
      ...
      ...(target ? { TargetCharacterName: target } : {}),
    };

    onSendMessage?.();
    setSending(true);

    const updateTargetMessage = updateAssistantResponse(
      updateBotMessageIdRef.current,
    );
    ...
    try {
      ...
      if (error instanceof EventSourceOpenFail) {
        ...
        updateTargetMessage(
          genSpecificMessage.error(
            EMessageType.Error,
            error?.message ||
              botErrorCodesMessageMap.ConnectionEstablishmentFailed,
            error.logid,
          ),
        );
      ...
      if (isNormalSSEError(error)) {
        if (isTextRiskError(error)) {
          const {
            ...
            textRiskTypeContentMap[
              error.code as keyof typeof textRiskTypeContentMap
            ] || {};

          updateTargetMessage(
            genSpecificMessage.error(
              type as EMessageType.Message | EMessageType.Error,
              content,
            ),
          );
        } else {
          updateTargetMessage(
            genSpecificMessage.error(
              EMessageType.Error,
              getErrorContent(error as SSEError),
              error.logid,
            ),
          );
        }
      ...
      updateTargetMessage(
        genSpecificMessage.error(
          EMessageType.Error,
          (error as any)?.message || '发生意外错误，请联系管理员',
          (error as any)?.logid,
        ),
      );
  ...
  const retryMessage = () => {
    ...
    retryBotMessage.versions.push([
      genSpecificMessage.loading(),
      genSpecificMessage.slot(newMessageId),
    ]);
    ...
    const { name } = retryBotMessage.sender;
  ...
  const removeLoadingCardInVersions = (versionIdx: number) => {
    ...
    if (!lastAssistantMsg) {
      throw new Error(
        'removeLoadingCardInVersions: last bot message not found',
      );
    }
    ...
    const processedVersion = targetVersion.reduce(
