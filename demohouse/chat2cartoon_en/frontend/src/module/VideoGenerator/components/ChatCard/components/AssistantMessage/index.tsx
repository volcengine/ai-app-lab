...
const AssistantMessage = (message: AssistantMessageProps) => {
  ...
  const handleNext = async () => {
    ...
    if (topMessage.phase === VideoGeneratorTaskPhase.PhaseScript) {
      sendNextMessage('Generate storyboard', false);
    } else {
      sendNextMessage('Generate video', false);
    }
  };
  ...
  return (
    ...
    >
      {isContentFilter ? (
        <MessageContent message={'Unknown Error'} isAnimate={!topMessage.finish} />
      ) : (
      ...
      {isLengthExceed ? (
        <Alert
          ...
          type="warning"
          content={
            'The output may be incomplete because the maximum number of tokens is reached. We recommend that you shorten the input or clear the context before trying again.'
          }
        />
      ) : null}
