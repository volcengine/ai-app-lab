...
async def api_model_list():
    ...
    models = [{"name": model_name, "display_name": model_detail.display_name}
              for model_name, model_detail in settings.models.items()]
...
async def handle_task(sandbox_id, task_id, model_name,
    ...
    async with sse_client(mcp_server_endpoint) as streams:
        async with ClientSession(*streams) as mcp_session:
            await mcp_session.initialize()
            ai_client = OpenAI(api_key=get_settings().models.get(model_name).api_key,
                               base_url=get_settings().models.get(model_name).base_url)
