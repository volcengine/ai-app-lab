...
export const LeftPanel: FC = () => {
  const { messages, inputMessage, ip, modelName, modelList } = useSnapshot(
    store,
    {
      sync: true,
    }
  ...
  // 判断聊天窗口是否可用（需要有IP）
  const isChatDisabled = !ip || !modelName;
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3">
        {messages.length === 0 ? (
          <div className="flex items-center justify-center h-full text-slate-400 text-sm">
            <p>
              {isChatDisabled ? "请先选择或创建一个实例" : "开始新的对话吧"}
            </p>
          </div>
        ) : (
            ...
            <div className="rounded-md w-full">
              <div className="relative rounded-md p-[1px] bg-gradient-to-r from-[#8AC5FF] via-[#74ABFF] to-[#E1A9FF]">
                <Input.TextArea
                  ...
                  placeholder={
                    isChatDisabled ? "请先选择一个实例..." : "问我任何问题吧！"
                  }
                  value={inputMessage}
