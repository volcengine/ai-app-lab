...
from .base import BaseError, ignore_extra_kwargs, wrap_pyautogui_async, camel_to_snake
...
class PyAutoGUIComputerTool(IComputerTool):
    ...
    async def drag_mouse(self, r: DragMouseRequest):
        ...
        await self.press_mouse(drag_path[0][0], drag_path[0][1], "left")
        for loc in drag_path:
            time.sleep(0.2)
            await self.move_mouse(loc[0], loc[1])
        time.sleep(0.2)
        return await self.release_mouse(drag_path[-1][0], drag_path[-1][1], "left")
    ...
    def scroll(self, r: ScrollRequest):
        ...
        x, y = r.x, r.y
        logger.info(f"scroll in windows, amount: {scroll_amount}, direction: {scroll_direction}")
        if isinstance(scroll_amount, str):
            scroll_amount = int(scroll_amount) * 100
    ...
    @wrap_pyautogui_async
    def take_screenshot(self, r: TakeScreenshotRequest):
    ...
    async def change_password(self, r: ChangePasswordRequest):
        return change_password(r.username, r.new_password)
